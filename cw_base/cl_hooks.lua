...
local function CW20_RenderScreenspaceEffects()
	...
	if curTime > ( ply.cwFlashbangDuration or 0 ) then
		ply.cwFlashbangIntensity = math.Approach(ply.cwFlashbangIntensity or 0, 0, frameTime)
	end
	
	ply.cwFlashbangDisplayIntensity = math.Approach(ply.cwFlashbangDisplayIntensity or 0 , ply.cwFlashbangIntensity, frameTime * 15)
	
	if curTime > ( ply.cwFlashDuration or 0 ) then
		ply.cwFlashIntensity = math.Approach(ply.cwFlashIntensity or 0, 0, frameTime)
	end
	
	if ply.cwFlashbangDisplayIntensity > 0 then
		DrawMotionBlur(0.01 * (1 - ply.cwFlashbangDisplayIntensity or 0), ply.cwFlashbangDisplayIntensity or 0, 0)
		
		surface.SetDrawColor(255, 255, 255, 255 * ( ply.cwFlashIntensity or 0 ) * ply.cwFlashbangDisplayIntensity or 0)
		surface.DrawRect(-1, -1, ScrW() + 2, ScrH() + 2)
	end
end
